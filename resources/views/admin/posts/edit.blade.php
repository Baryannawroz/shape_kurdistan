                                    ...
                                    @foreach(config('app.locales') as $locale => $details)
                                        <div x-show="lang === '{{ $locale }}'" style="display: none;" :style="lang === '{{ $locale }}' ? 'display: block' : 'display: none'">
                                            <div class="mb-4">
                                                ...
                                                <input type="text" name="title[{{ $locale }}]" value="{{ $post->getTranslation('title', $locale, false) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                            </div>
                                            <div class="mb-4">
                                                <label class="block text-gray-700 text-sm font-bold mb-2">{{ __('Excerpt') }} ({{ $locale }})</label>
                                                <textarea name="excerpt[{{ $locale }}]" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" rows="3">{{ $post->getTranslation('excerpt', $locale, false) }}</textarea>
                                            </div>
                                            <div class="mb-4">
                                                <label class="block text-gray-700 text-sm font-bold mb-2">{{ __('Body') }} ({{ $locale }})</label>
                                                <textarea name="body[{{ $locale }}]" id="editor-{{ $locale }}" class="rich-editor w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ $post->getTranslation('body', $locale, false) }}</textarea>
                                            </div>
                                            ...
                                            <div class="mt-6 border-t pt-4">
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <input type="text" name="meta_title[{{ $locale }}]" value="{{ $post->getTranslation('meta_title', $locale, false) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                                </div>
                                                <div class="mb-4">
                                                    <label class="block text-gray-700 text-sm font-bold mb-2">{{ __('Meta Description') }}</label>
                                                    <textarea name="meta_description[{{ $locale }}]" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" rows="2">{{ $post->getTranslation('meta_description', $locale, false) }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                            ...
                            <div class="col-span-1 space-y-6">
                                <div class="bg-gray-50 p-4 rounded shadow-sm border">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <input type="datetime-local" name="published_at" id="published_at" value="{{ $post->published_at ? $post->published_at->format('Y-m-d\TH:i') : '' }}" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                    </div>

                                    <div class="mb-4">
                                        <label class="inline-flex items-center">
                                            <input type="checkbox" name="allow_comments" value="1" {{ $post->allow_comments ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-blue-600">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <select name="section_id" id="section_id" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                            ...
                                            @foreach($sections as $section)
                                                <option value="{{ $section->id }}" {{ $post->section_id == $section->id ? 'selected' : '' }}>{{ $section->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <div class="max-h-32 overflow-y-auto border rounded p-2 bg-white">
                                            @foreach($tags as $tag)
                                                <div class="flex items-center">
                                                    <input type="checkbox" name="tags[]" value="{{ $tag->id }}" id="tag_{{ $tag->id }}" {{ $post->tags->contains($tag->id) ? 'checked' : '' }} class="mr-2">
                                ...
                                <div class="bg-gray-50 p-4 rounded shadow-sm border">
                                    ...
                                    @if($post->featured_image)
                                        <img src="{{ asset('storage/' . $post->featured_image) }}" alt="Featured" class="w-full h-32 object-cover rounded mb-2">
                                    @endif
