...
const form = useForm({
    ...
    translations: props.locales.map((loc) => {
        const t = props.service?.translations?.[loc] ?? {};
...
function submit() {
    if (props.service?.id) {
        form.put(r('admin.cms.services.update', props.service.id), { forceFormData: true });
    } else {
        form.post(r('admin.cms.services.store'), { forceFormData: true });
    }
}
</script>
...
<template>
    <AdminLayout>
        ...
        <h1 class="text-2xl font-bold">{{ service ? 'Edit service' : 'New service' }}</h1>
        <form class="mt-6 space-y-6 rounded-lg bg-white p-6 shadow" @submit.prevent="submit">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label class="text-sm font-medium">Icon (file)</label>
                    <input type="file" class="mt-1 w-full text-sm" @input="form.icon = $event.target.files[0]" />
                </div>
                <div>
