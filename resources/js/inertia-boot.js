...
export function removeLoadingFallback() {
...
export function showLoadFailure(message) {
    ...
    panel.innerHTML = `<div><p style="font-weight:600;margin-bottom:0.5rem;">Unable to load the page</p><p style="color:#475569;max-width:28rem;">${message}</p><p style="margin-top:1rem;color:#64748b;font-size:0.875rem;">Try a hard refresh (Ctrl+F5). If this continues, ask your administrator to run <code>npm run build</code> on the server.</p></div>`;
    document.body.prepend(panel);
}

/**
 * @param {Record<string, () => Promise<{ default: import('vue').Component }>>} pageModules
 */
export function bootInertia(pageModules) {
    createInertiaApp({
        resolve: async (name) => {
